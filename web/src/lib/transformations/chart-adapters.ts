/**
 * Chart adapter utilities – converts domain data to Lightweight Charts format.
 *
 * All chart-library-specific types and formatting logic lives here,
 * keeping domain/math logic in `inflation.ts` and shared helpers in `common.ts`.
 * Moved from the former `chart-utils.ts` as part of REF-003.
 */
...
>(data: T[]): ChartDataPoint[] {
  return sortByDate(filterValidValues(data))
    .map((item) => ({
...
>(data: T[]): OHLCDataPoint[] {
  return sortByDate(filterValidOHLC(data))
    .map((item) => ({
...
export function createIndicatorSeries<
  T extends { date: string | number; sma?: number | null; ema?: number | null }
>(
  data: T[],
  options: {
    showSMA?: boolean;
  ...
  if (showSMA) {
    const smaData = data
      ...
      .filter(
        (d): d is ChartDataPoint => d.time !== null && isFinite(d.value)
      );
    if (smaData.length > 0) {
      series.push({
        data: smaData,
...
export function getRiskBandColor(riskScore: number): string {
  ...
  const index = Math.min(
    Math.floor(riskScore * colors.length),
    colors.length - 1
  );
  return colors[index];
}
