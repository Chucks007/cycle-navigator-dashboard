...
- `types.ts`: Shared domain types (`SeriesPoint`, `OHLCSeriesPoint`).
- `common.ts`: Shared helpers for sorting/filtering and alignment-safe transforms.
...
## Implementation Plan
1. [x] Create `web/src/lib/transformations/` directory.
2. [x] Move mathematical/alignment logic into dedicated files (`common.ts`, `inflation.ts`, `chart-adapters.ts`).
3. [x] Update existing hooks (`use-inflation-adjusted-data.ts`) to use the new transformation library.
4. [x] Ensure unit tests are moved and updated for the new structure.
